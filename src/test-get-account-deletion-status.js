...
module.exports = async function (credentials) {
  ...
  if (body) {
    apiCall.addBodyCheck(
      "returns 'deletionStatus: DONE' when account does not exist - received " +
        body.deletionStatus,
  ...
  if (body) {
    apiCall.addBodyCheck(
      "returns 'deletionStatus: CANCELED' or 'deletionStatus: PENDING' - received " +
        body.deletionStatus,
