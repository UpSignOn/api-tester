...
const { get } = require('./report-builder/helpers');
...
const testButtonConfigResponse = async (testGroup, queryParameters, config) => {
  ...
  if (body) {
    ...
    apiCall.addBodyCheck(
      "'generalConfigVersion' is of type string",
      typeof body.generalConfigVersion === 'string',
    );
    apiCall.addBodyCheck(
...
module.exports = async function () {
  ...
  const apiCall = testGroup.newApiCall('GET', '/button-config', 'for unknown buttonId', {
    buttonId: 'unknown',
  });
