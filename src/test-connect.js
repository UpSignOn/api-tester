...
const testConnectErrorCases = async () => {
  ...
  apiCall = testGroup.newApiCall('POST', '/connect', 'when userId is missing', {
    password: 'password',
  });
  await apiCall.checkStatus([400, 401]);

  apiCall = testGroup.newApiCall('POST', '/connect', 'when password is missing', {
    userId: 'badId',
  });
...
const testConnect = async (credentials) => {
  ...
  let apiCall = testGroup.newApiCall(
    'POST',
    '/connect',
    'when password does not match a correct userId',
    {
  ...
  if (body) {
    ...
    if (body.redirectionUri) {
      try {
        ...
        apiCall.addBodyCheck(
          "'redirectionUri' should not return a 200 on GET without token - received " +
            uriResponse.status,
        ...
        apiCall
          .security()
          .addBodyCheck(
            "'redirectionUri' should not return a 200 on GET with a bad token - received " +
              uriResponse.status,
            uriResponse.status !== 200,
          );
        uriResponse = await head(redirectionWithGoodToken);
        apiCall.addBodyCheck(
          "'redirectionUri' should return a 200 on GET with a good token - received " +
            uriResponse.status,
          uriResponse.status === 200,
        );
        uriResponse = await head(redirectionWithGoodToken);
        apiCall
          .security()
          .addBodyCheck(
            "'redirectionUri' should not return a 200 on GET with a reused token - received " +
              uriResponse.status,
            uriResponse.status !== 200,
          );
      } catch {
