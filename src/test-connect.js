...
const testConnectErrorCases = async () => {
  ...
  checkSecurity("returns a 400 or 401 when password is missing", response.status === 401 || response.status === 400);

  response = await post("/connect", { body: JSON.stringify({ userId: "badId", password: "badPassword" }) });
  checkSecurity("returns a 401 whith bad credentials", response.status === 401);
};
...
const testConnect = async (credentials) => {
  ...
  if (body) {
    ...
    checkSecurity("'redirectionUri' should not return a 200 on GET with a reused token", uriResponse.status !== 200);
  }
};
